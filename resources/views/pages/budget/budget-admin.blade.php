...
<style>
    ...
    /* Center semua kolom No */
    .table-budget-admin th:nth-child(1),
    .table-budget-admin td:nth-child(1) {
        text-align: center;
        vertical-align: middle;
    }

    /* Center semua kolom action (View buttons) */
    .table-budget-admin th:not(:nth-child(2)),
    .table-budget-admin td:not(:nth-child(2)) {
        vertical-align: middle;
    }

    /* Wrapper tombol agar benar-benar center */
    .btn-cell {
        display: flex;
    ...
    /* Struktur tetap kiri */
    .table-budget-admin th:nth-child(2),
    .table-budget-admin td:nth-child(2) {
        text-align: left;
    }
</style>
    ...
    function renderBudgetTable(data) {
        ...
        data.forEach(function(row) {
            ...
            const companyPolicyCell = row.company_policy_url ?
                `<div class="btn-cell"><a href="${row.company_policy_url}" class="btn btn-sm btn-outline-primary btn-view-link">View</a></div>` :
                '<span class="text-muted">-</span>';

            const kpiCell = row.kpi_url ?
                `<div class="btn-cell"><a href="${row.kpi_url}" class="btn btn-sm btn-outline-success btn-view-link">View</a></div>` :
                '<span class="text-muted">-</span>';

            const workplanCell = row.workplan_url ?
                `<div class="btn-cell"><a href="${row.workplan_url}" class="btn btn-sm btn-outline-info btn-view-link">View</a></div>` :
                '<span class="text-muted">-</span>';

            const planBudgetCell = row.plan_budget_url ?
                `<div class="btn-cell"><a href="${row.plan_budget_url}" class="btn btn-sm btn-outline-warning btn-view-link">View</a></div>` :
                '<span class="text-muted">-</span>';

            const budgetCell = row.budget_url ?
                `<div class="btn-cell"><a href="${row.budget_url}" class="btn btn-sm btn-outline-danger btn-view-link">View</a></div>` :
                '<span class="text-muted">-</span>';
