                            ...
                            <div class="col-md-3">
                                ...
                                <select name="budget_code" class="form-select form-select-sm" id="budget-code-filter">
                                    ...
                                    @foreach($budgetCodes as $code)
                                        <option value="{{ $code->stock_code }}" {{ $budgetCode == $code->stock_code ? 'selected' : '' }}>
                            ...
                            <tbody>
                                @forelse($budgetData as $divisionName => $items)
                                    ...
                                    <tr class="division-header">
                                        <td rowspan="{{ count($items) + 1 }}"><strong>{{ $divisionName }}</strong></td>
                                        <td colspan="3"><strong>TOTAL {{ strtoupper($divisionName) }}</strong></td>
                                        <td class="text-end"><strong>{{ number_format($divisionTotalBudget, 2) }}</strong></td>
                                    ...
                                    @foreach($items as $item)
                                        <tr>
                                            ...
                                            @foreach(['jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec'] as $month)
                                                <td class="text-end">{{ number_format($item['months'][$month], 0) }}</td>
                                                <td class="text-end">0</td>
                                                <td class="text-end">{{ number_format($item['months'][$month], 0) }}</td>
                                            @endforeach
                                        </tr>
                                    ...
                                    <tr>
                                        <td colspan="43" class="text-center py-4">
                                            <div class="text-muted">
                                                ...
                                                <p class="mt-2">No budget data available for the selected filters.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
