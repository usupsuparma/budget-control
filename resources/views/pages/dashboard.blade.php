...
@section('title', 'Dashboard | Budget Control')
...
(function () {
    ...
    const URLS = {
        ...
        division   : "{{ route('dashboard.division.realization') }}",
        monthly    : "{{ route('dashboard.monthly.chart') }}",
    };
    ...
    function loadStats(year) {
        ...
        $.get(URLS.stats, { year }, function (res) {
    ...
    function initChart(labels, budgetSeries, realizationSeries) {
        const opts = {
            ...
            series: [
                { name: 'Budget',      type: 'bar',  data: budgetSeries },
                { name: 'Realisasi',   type: 'line', data: realizationSeries },
            ],
        ...
        if (budgetChart) {
            budgetChart.updateOptions({ xaxis: { categories: labels } });
            budgetChart.updateSeries([
                { name: 'Budget',    data: budgetSeries },
                { name: 'Realisasi', data: realizationSeries },
            ]);
        } else {
    ...
    function loadMonthlyChart(year) {
        $.get(URLS.monthly, { year }, function (res) {
            if (!res.success) return;
            const d = res.data;
    ...
    function loadDivisionTable(year) {
        ...
        $.get(URLS.division, { year }, function (res) {
            if (!res.success) {
